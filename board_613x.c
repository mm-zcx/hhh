...
char autoinit_check(void) {
	...
	// check the Hardware Pending Int register for auto-initialization errors
	 ...
	 read_value = Read_6131LowReg(HDW_PENDING_INT_REG,1);            
...
void initialize_613x_shared(void) {
	...
	// or we use SPI to initialize HI-6131 registers and RAM tables     
                unsigned short j = Read_6131LowReg(MASTER_CONFIG_REG, 0);
		...
		// elsewhere. The following options may be selected here:
                ...
                j |= PULSE_INT; // |TXINHA|TXINHB;
		Write_6131LowReg(MASTER_CONFIG_REG, j, 0);					

		// enable these hardware interrupts: 
                // EEPROM checksum error
                // auto-init RAM init fail (mismatches EEPROM)
                // bus A and bus B loopback failures
	        j = EECKE_INT|RAMIF_INT|LBFA|LBFB;
                Write_6131LowReg(HDW_INT_ENABLE_REG, j, 0);
...
void write_init_eeprom(void) {
	...
	if ((SMT_ena|IMT_ena ==1) && (SMT_IMT_autostart==1)) {	
		...
		code2 ^= 0x0300;
            ...
            // main() will re-enable them later
            __disable_interrupt(); 
            // enable Memory Address Pointer 1
            enaMAP(1);
	    // init memory address pointer
	    Write_6131LowReg(MAP_1,0x004E,0);
	    // write unlock code #1 to RAM address 0x004E
	    Write_6131_1word(code1,0);
	    // init memory address pointer again (it auto-incremented)
	    Write_6131LowReg(MAP_1,0x004E,0);
	    // write unlock code #2 to RAM address 0x004E
	    Write_6131_1word(code2,0);
	//#endif
